...
## ????? ??AWS (App Runner)
??????? ???? `apprunner.yaml` ?????? ????? ??GitHub.

### ???? ????? ??????
1. Push ???? ??GitHub (`main`).
2. ??AWS App Runner ??? `Create service` -> `Source code repository`.
3. ??? ?? ?????: `Liranmessinger/ai-news-studio`.
4. ??? `Use configuration file` (App Runner ???? ?? `apprunner.yaml`).
